 ...
 */
void esp32_set_ready(struct esp32state *inst,
                     void (*on_ready)(struct esp32state *)) {
  inst->on_ready = on_ready;
}

void esp32_set_receive(struct esp32state *inst,
...
/**
 * Processes the queue
 * TODO rename the function? `run' implies too much, whereas process is more
 * apt?
 * @param inst The ESP32 state instance
 */
void esp32_run_queue(struct esp32state *inst) {
  ...
  if (inst->active_socket != NULL) {
    if (inst->active_socket->receivedBytesRemaining == 0) {
      // done receiving, remove from being the active socket and call a
      // receive() function
...
void esp32_enable_uart(struct esp32state *inst) {
  ...
  *(uint32_t *)UART0_CTRL_ADDR = 0b11;
  ...
  while ((*(uint32_t *)UART0_CTRL_ADDR) ==
         0b11) { /* wait for reset, should implement a counter here for a
                    timeout */
  }
