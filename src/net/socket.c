...
void socket_init(struct esp32state *inst, struct at_socket *sock,
                 enum at_socket_protocol type, char host[SOCKET_HOST_SIZE],
                 int port) {
...
void socket_open(struct esp32state *inst, struct at_socket *sock) {
  char buffer[64];
  sprintf(buffer, "AT+CIPSTART=%d,\"%s\",\"%s\",%d", sock->link_id,
          AT_SOCKET_PROTOCOLS_STR[sock->protocol], sock->host, sock->port);
...
void socket_send(struct esp32state *inst, struct at_socket *sock, char *data,
                 int length) {
  char buffer[32];
...
void socket_set_receive(struct at_socket *sock,
                        void (*cb)(void *, struct at_socket *)) {
  sock->on_receive = cb;
}
